...
## Background-Init Absorption Test
I added and ran:
- `data/HashINR/scripts/test_background_init_absorption.py`

Requested device:
- GPU0 for the current task
- I also verified the same loss result on physical GPU1 by isolating it with `CUDA_VISIBLE_DEVICES=1`

Output:
- `data/HashINR/outputs/background_init_absorption_test_cuda0_diagnosed.json`
- `data/HashINR/outputs/background_init_absorption_test_cuda1_visible.json`

Test definition:
- evaluate the initial framewise k-space data-consistency loss before any backpropagation
- compare:
  - normal `I_init` plus the randomly initialized trainable branches
  - zero `I_init` plus the same randomly initialized trainable branches
  - `I_init` only with trainable branches zeroed
  - zero init and zero trainable branches

Results:
- `loss_with_I_init_and_random_branches = 2907.4620`
- `loss_without_I_init_and_random_branches = 2115.4525`
- `loss_with_I_init_only = 2907.4620`
- `loss_with_zero_init_and_zero_branches = 2115.4525`
- relative reduction with `I_init`:
  - `-37.44%`

Interpretation:
- under this criterion, the current `I_init` does **not** lower the initial framewise data-consistency loss
- it raises the initial loss by about `37.4%` relative to zero initialization
- the near equality between:
  - `I_init + random branches`
  - and `I_init only`
  shows that the learned branches are essentially negligible at initialization, so this is directly measuring the effect of `I_init`

Conclusion:
- if the criterion is:
  - “does `I_init` already absorb the dominant signal energy in a way that reduces the initial framewise k-space loss?”
  then the answer is currently **no**
- the present background initialization is acting more like a strong static prior than a lower-loss framewise measurement fit

### Likely problem identified
The strongest current evidence points to a calibration mismatch rather than a wrong spatial pattern.

From the GPU0 diagnostic:
- `averagedBG` versus the mean of `Global_CG.npy`
  - magnitude correlation: `0.9887`
  - best-fit complex scale magnitude: `1.7332`
  - best-fit phase shift: essentially `0 deg`
  - relative residual after best scalar fit: `0.1698`

Interpretation:
- the background image geometry is largely correct
- but its amplitude is substantially mis-scaled relative to the current training forward model and the cufinufft-based reconstruction reference

Most likely causes:
- `averagedBG.npy` is built in `DataSimulation.ipynb` with `gpunufft`, while the current training config uses `cufinufft`
- the notebook uses `pinv_solver(..., max_iter=25)` on accumulated data, which may be under-converged and amplitude-biased
- the resulting image is then used as a fixed background inside a different forward operator during training

So the current issue appears to be:
- not “the averaged accumulated background idea is wrong”
- but “the saved `averagedBG.npy` is not normalized consistently with the current training measurement model”

### Additional root-cause refinement
I traced the notebook background generation more closely and found a second likely bug in `data/HashINR/DataSimulation.ipynb`.

The background cells were building the accumulated trajectory as:
- `[1, total_samples, 3]`

for a single-coil solve, instead of:
- `[total_samples, 3]`

For a static single-coil background reconstruction, the consistent shapes are:
- k-space: `[n_coils, total_samples]`
- samples: `[total_samples, 3]`

That notebook shape mismatch is a strong candidate for the remaining amplitude/phase error in `averagedBG.npy`.

I patched the notebook cells so the accumulated background now uses:
- `cufinufft`
- `Kspace_acculumulted.shape == [n_coils, total_samples]`
- `traj_stacked.shape == [total_samples, 3]`

## Notebook Launch Convention
For HashINR notebook work, use:
- `data/HashINR/scripts/start_jupyter_gpu0_port9005.sh`

This launcher:
- pins notebook work to physical GPU0
- uses port `9005`
- sets port retries to `0`, so it will not silently fall back to `9002`
